...
require "brains/dsaabigailbrain"
...
local function Retarget(inst)
	local wendy = GetWendy()
    local newtarget = FindEntity(inst, 20, function(guy)
            return  guy.components.combat and 
                    inst.components.combat:CanTarget(guy) and wendy ~= nil and
                    (guy.components.combat.target == wendy or wendy.components.combat.target == guy)
    end)
...
local function auratest(inst, target)
    ...
    if target == GetWendy() then return false end
...
local function fn(Sim)
    ...
    local brain = require "brains/dsaabigailbrain"
    inst:SetBrain(brain)
	...
	inst.components.inspectable.nameoverride = "abigail"
    ...
    inst.components.lootdropper:SetLoot({"dsaabigail_flower"})
    ------------------    
	...
	local wendy = GetWendy()
	if wendy and wendy.components.leader then
		inst.components.follower.maxfollowtime = 99999999
		wendy.components.leader:AddFollower(inst)
...
return Prefab( "common/monsters/dsaabigail", fn, assets, prefabs ) 
