...
require "brains/ynaabigailbrain"
...
local function Retarget(inst)
    ...
    local newtarget = FindEntity(inst, 20, function(guy)
            return  guy.components.combat and 
                    ...
                    (guy.components.combat.target == GetWendy() or GetWendy().components.combat.target == guy)
    end)
...
local function fn(Sim)
    ...
    local brain = require "brains/ynaabigailbrain"
    inst:SetBrain(brain)
    ...
    inst.components.lootdropper:SetLoot({"ynaabigail_flower"})
    ------------------    
...
return Prefab( "common/monsters/ynaabigail", fn, assets, prefabs ) 
