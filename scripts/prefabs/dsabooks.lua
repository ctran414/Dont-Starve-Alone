...
function sleepfn(inst, reader)
    ...
    for k,v in pairs(ents) do
        if v.components.sleeper and v ~= reader and not v:HasTag('summonedbyplayer') then
...
function MakeBook(name, usefn, bookuses )
    ...
    local function fn(Sim)
		...
		local model = string.sub(name, 4)
        anim:SetBank("books")
        anim:SetBuild("books")

        anim:PlayAnimation(model)
        MakeInventoryPhysics(inst)
        
        -----------------------------------
        
        inst:AddComponent("inspectable")
		inst.components.inspectable.nameoverride = model
        inst:AddComponent("book")
        inst.components.book.onread = usefn
        
        inst:AddComponent("inventoryitem")
		inst.components.inventoryitem:ChangeImageName(model)
...
return MakeBook("dsabook_sleep", sleepfn, 5),
       MakeBook("dsabook_gardening", growfn, 5),
       MakeBook("dsabook_brimstone", firefn, 5),
       MakeBook("dsabook_birds", birdsfn, 3),
       MakeBook("dsabook_tentacles", tentaclesfn, 5)
